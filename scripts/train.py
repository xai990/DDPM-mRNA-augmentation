...
def main():   
    logger.configure(format_strs = '%(asctime)s - %(levelname)s - %(message)s')
    logger.log("**********************************")
    ...
    for i in range(num_shows):
        j = i // 10
        k = i % 10
        q_i = DF.q_sample(dataset, th.tensor([i*steps_num//num_shows])).cpu()
        '''if i == 0:
            mean, var = DF.q_xt_x0(dataset,torch.tensor([i*steps_num//num_shows]))
        ...
        axs[j,k].set_title('$q(\mathbf{x}_{'+str(i*steps_num//num_shows)+'})$')
